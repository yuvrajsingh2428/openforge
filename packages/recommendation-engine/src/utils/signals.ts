...
export function extractSignals(issue: Issue, category: string): ScoringSignals {
  ...
  return {
    ...
    issueLabels: labels.filter(Boolean).map((l) => l.name?.toLowerCase() ?? ""),
    issueLabelColors: labels.filter(Boolean).map((l) => ({ name: l.name ?? "", color: l.color ?? "" })),
    ...
    repoStars: repo?.stargazerCount ?? 0,
    repoForks: repo?.forkCount ?? 0,
