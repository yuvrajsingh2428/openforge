...
interface IssueErrorStateProps {
  message?: string;
  onRetry?: () => void;
}

export function IssueErrorState({ message, onRetry }: IssueErrorStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <p className="text-sm text-muted-foreground max-w-sm mb-4">
        {message ?? "We couldn't load issues. This might be a temporary issue with the GitHub API."}
      </p>
      {onRetry && (
        <button
          onClick={onRetry}
