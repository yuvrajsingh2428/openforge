 ...
 * /api/ai/summary:
 *   post:
 *     summary: Generate summary of issue
 *     responses:
 *       200:
 *         description: Success
 */
export async function POST(request: Request) {
  ...
  try {
    const analysis = await generateIssueSummary(result.data);
    if (!analysis.success) {
    ...
    return errorResponse(sanitizeError(error, "Failed to generate issue summary"), 500);
  }
}
